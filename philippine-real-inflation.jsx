...
const { AreaChart, Area, LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, BarChart, Bar, Cell } = Recharts;

// ── DATA — Sources: PSA CPI (2015–2025), World Bank WDI (2000–2014), BSP USD/PHP ─
const OFFICIAL = {
  // World Bank / PSA historical (2018=100 rebased) — source: data.worldbank.org + psa.gov.ph
...
const MONTHS_2026 = MONTHLY; // Jan-Apr 2026
...
// ── SAVINGS — mo=1-12, fractional first year for accuracy
function calcSavings(amt, yr, mo, rate) {
  const frac = (12 - mo) / 12; // portion of deposit year remaining
  ...
  const totalMos = (2025 - yr) * 12 + (12 - mo + 12);
  return {
    rows, L,
...
// ── THEME ─────────────────────────────────────────────────────────────────────
const L = { bg:"#ffffff", surface:"#f4f4f5", border:"#e4e4e7", text:"#09090b", heading:"#18181b", muted:"#71717a", faint:"#d4d4d8", red:"#dc2626", blue:"#1d4ed8", gold:"#b45309", green:"#15803d", tip:"#ffffff", inp:"#f4f4f5", inpB:"#d4d4d8", tabB:"#71717a" };
const D = { bg:"#09090b", surface:"#18181b", border:"#27272a", text:"#ffffff", heading:"#ffffff", muted:"#d1d5db", faint:"#52525b", red:"#ef4444", blue:"#60a5fa", gold:"#fbbf24", green:"#4ade80", tip:"#18181b", inp:"#27272a", inpB:"#52525b", tabB:"#71717a" };
...
function drawNumberCard(canvas, off, real, month, isDark) {
  if (!canvas) return;
  ...
  ctx.fillText("pri.netlify.app",1156,62);
  ctx.textAlign="left";
...
function drawSavingsCard(canvas, amt, yr, bank, rPSA, rReal, hidden, yrs, isDark) {
  if (!canvas) return;
  canvas.width=1200; canvas.height=630;
  const ctx=canvas.getContext("2d");
  ctx.fillStyle="#f7f7f4"; ctx.fillRect(0,0,1200,630);
  ...
  g.addColorStop(0,"#1d4ed8"); g.addColorStop(1,"#dc2626");
  ctx.fillStyle=g; ctx.fillRect(0,0,1200,5);
  drawFlag(ctx,44,22,60,40);
  ctx.fillStyle="#0d0d0d"; ctx.font="bold 44px system-ui";
  ctx.fillText("PHILIPPINE REAL INFLATION",118,58);
  const tw=ctx.measureText("PHILIPPINE REAL INFLATION  ").width;
  ctx.fillStyle="#9ca3af"; ctx.font="bold 28px system-ui";
  ctx.fillText("SAVINGS",118+tw,58);
  ctx.textAlign="right"; ctx.fillStyle="#9ca3af"; ctx.font="26px system-ui";
  ctx.fillText("pri.netlify.app",1156,58);
  ctx.textAlign="left";
  ctx.fillStyle="#374151"; ctx.font="28px system-ui";
  ctx.fillText("₱"+Number(amt).toLocaleString("en-PH")+" saved in "+yr+"  ·  "+yrs+" years",48,96);
  ctx.fillStyle="#e5e5e0"; ctx.fillRect(598,112,3,478); ctx.fillRect(0,368,1200,3);
  const bankInt=bank-amt;
  const psaLoss=amt-rPSA;
  const realLoss=amt-rReal;
  const cells=[
    {label:"BANK BALANCE",val:peso(bank),  sub:bankInt>=0?"+"+peso(bankInt):peso(bankInt),numC:"#0d0d0d",lblC:"#6b7280",subC:"#374151",x:0,  y:112},
    {label:"PSA SAYS",    val:peso(rPSA),  sub:"−"+peso(psaLoss),                   numC:"#0d0d0d",lblC:"#6b7280",subC:"#6b7280",x:601,y:112},
    {label:"ACTUALLY",    val:peso(rReal), sub:"−"+peso(realLoss),                  numC:"#0d0d0d",lblC:"#6b7280",subC:"#6b7280",x:0,  y:371},
    {label:"HIDDEN LOSS", val:"−"+peso(Math.abs(hidden)),sub:"what PSA omits",       numC:"#dc2626",lblC:"#dc2626",subC:"#dc2626",x:601,y:371},
  ];
  cells.forEach(cell=>{
    ctx.fillStyle=cell.lblC; ctx.font="bold 26px system-ui";
    ctx.fillText(cell.label,cell.x+48,cell.y+48);
    let fs=108; ctx.font=`bold ${fs}px system-ui`;
    while(ctx.measureText(cell.val).width>480&&fs>30){fs-=4;ctx.font=`bold ${fs}px system-ui`;}
    ctx.fillStyle=cell.numC; ctx.fillText(cell.val,cell.x+48,cell.y+48+fs+10);
    ctx.fillStyle=cell.subC; ctx.font="24px system-ui";
    ctx.fillText(cell.sub,cell.x+48,cell.y+48+fs+44);
  });
  ctx.fillStyle="#e5e5e0"; ctx.fillRect(0,578,1200,2);
  ctx.fillStyle="#9ca3af"; ctx.font="22px system-ui";
  ctx.fillText("PSA Official  +  market rice  +  peso depreciation passthrough",48,614);
}
...
function App() {
  ...
  // keep first entry for canvas & legacy usage
  ...
  const amt=entries[0].amt, yr=entries[0].yr, mo=entries[0].mo, rate=entries[0].rate;
  ...
  // Auto-draw now card when on now tab
  useEffect(() => {
    if (tab === "now" && nowCanvasRef.current) {
      drawNumberCard(nowCanvasRef.current, cur.off, cur.real, cur.label, dark);
    }
  ...
  const ShareBar = ({ canvasRef }) => (
    <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
      ...
      <button onClick={() => download(canvasRef)} style={{
        padding:"8px 20px", borderRadius:999,
        border:`1.5px solid ${c.blue}`, background:"transparent",
        color:c.blue, fontWeight:700, cursor:"pointer", fontSize:13,
      }}>⬇ Download</button>
      ...
      <div style={{ borderBottom:`1px solid ${c.border}`, padding:"16px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
          ...
          <div onClick={()=>setDark(d=>!d)} style={{ display:"flex", alignItems:"center", gap:6, cursor:"pointer", userSelect:"none" }}>
            <span style={{ fontSize:11, color:c.heading }}>☀️</span>
            ...
            <span style={{ fontSize:11, color:c.heading }}>🌙</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:860, margin:"0 auto", padding:"28px 20px 80px" }}>
        ...
        {tab==="now" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            {/* Period selector — shows current, click to change */}
            <div style={{ position:"relative" }}>
              ...
              {showPicker && (
                <div style={{
                  ...
                  {/* Historical years */}
                  <div style={{ fontSize:10, fontWeight:700, color:c.muted, letterSpacing:1, marginBottom:6 }}>YEAR <span style={{ fontWeight:400, textTransform:"none", letterSpacing:0 }}>(no monthly data)</span></div>
            ...
            {/* Hero numbers — outer border shows this is what gets downloaded */}
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr" }}>
                <div style={{ background:c.surface, borderRight:`2px solid ${c.heading}`, padding:"24px 24px 20px" }}>
                  <div style={{ display:"flex", alignItems:"baseline", gap:5, marginBottom:12 }}>
                    <span style={{ fontSize:11, fontWeight:700, letterSpacing:2, color:c.muted }}>OFFICIAL</span>
                    <span style={{ fontSize:10, color:c.faint }}>(gov)</span>
                  </div>
                ...
                <div style={{ background:dark?"#1a0f00":"#fff7ed", padding:"24px 24px 20px" }}>
                  ...
                  <div style={{ fontSize:13, fontWeight:700, color:c.red, marginTop:12 }}>(+{gap}pp higher than official)</div>
                </div>
              </div>
            </div>
            ...
            {/* Copy + Download — stacked, centered */}
            ...
            {/* Comparative Area Chart */}
            <div style={{ background:c.surface, border:`1px solid ${c.border}`, borderRadius:16, padding:"20px 16px 14px" }}>
              <div style={{ fontSize:11, fontWeight:700, letterSpacing:2, color:c.heading, marginBottom:16, paddingLeft:8 }}>OFFICIAL (from gov) vs REAL (2000–2026)</div>
              ...
              <div style={{ display:"flex", gap:20, paddingLeft:8, marginTop:4 }}>
                {[[c.muted,"Official (from gov)"],[c.gold,"Real"]].map(([col,lbl])=>(
        ...
        {tab==="calc" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <div style={{ background:c.surface, border:`1px solid ${c.border}`, borderRadius:16, overflow:"hidden" }}>
                ...
                return (
                  <div key={e.id} style={{ borderBottom:`1px solid ${c.border}`, position:"relative" }}>
                    ...
                    <div style={{ padding:"12px 16px", display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                      ...
                      <div style={{ fontSize:10, color:c.muted, gridColumn:"1/-1" }}>
                        From {MONTHS[e.mo-1]} {e.yr} — {s.yrs} years applied
                      </div>
                    </div>
                    {/* Results row */}
              ...
              {/* Add entry */}
              <div style={{ padding:"12px 16px", borderBottom:entries.length>1?`1px solid ${c.border}`:"none" }}>
              ...
              {/* Totals — only when multiple entries */}
              {entries.length > 1 && (
            ...
            <div style={{ background:c.surface, border:`1px solid ${c.border}`, borderRadius:12, padding:"12px 18px", fontSize:12, color:c.text, lineHeight:1.7 }}>
              <b style={{ color:c.text }}>Why is "PSA Says" higher than "Actually"?</b> PSA's lower inflation shows less erosion. Real inflation is higher — your actual purchasing power is lower than the government claims.
            </div>
            ...
            <div style={{ background:dark?"#1a0505":"#fff1f1", border:`2px solid ${c.red}`, borderRadius:16, padding:"24px 28px", display:"flex", alignItems:"center", gap:24, flexWrap:"wrap" }}>
              ...
              <div style={{ fontSize:13, color:c.text, lineHeight:1.8, flex:1, minWidth:180 }}>
                Extra lost — your bank balance vs real purchasing power{entries.length>1?` across ${entries.length} entries`:``}. Changes with bank rate.
              </div>
            </div>
            ...
            <div style={{ background:c.surface, border:`1px solid ${c.border}`, borderRadius:16, padding:"20px 16px 14px" }}>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={combinedRows} margin={{ top:4,right:12,left:-10,bottom:4 }}>
                  ...
                  />
                  <Line type="monotone" dataKey="bank"  stroke={c.blue}  strokeWidth={2}   dot={false} name="bank"/>
                  <Line type="monotone" dataKey="rPSA"  stroke={c.muted} strokeWidth={1.5} dot={false} name="rPSA"/>
                  <Line type="monotone" dataKey="rReal" stroke={c.gold}  strokeWidth={3}   dot={false} name="rReal"/>
                </LineChart>
              </ResponsiveContainer>
        ...
        {tab==="how" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <div style={{ background:c.surface, border:`1px solid ${c.border}`, borderRadius:16, padding:"24px" }}>
              ...
              <div style={{ background:c.bg, borderRadius:10, padding:"20px 24px", fontFamily:"monospace", fontSize:14, color:c.text, lineHeight:2.6, border:`2px solid ${c.gold}`, boxShadow:`0 0 0 4px ${dark?"rgba(251,191,36,0.08)":"rgba(180,83,9,0.06)"}` }}>
                Real Inflation = PSA Official<br/>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;+ 0.5pp &nbsp;(market rice)<br/>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;+ peso dep% × 30% × 50%
              </div>
            </div>
            ...
            {[
              ...
              { title:"Market Rice Adjustment", add:"+0.5pp / yr", color:c.green, src:SOURCES.da,
                body:"PSA blends NFA subsidized rice prices with commercial prices when sampling. After the 2019 Rice Tariffication Law, NFA barely sells retail rice. Most Filipinos only access commercial rice, which costs more. We strip NFA pricing out.",
              ...
              { title:"Peso Depreciation Passthrough", add:"Variable", color:c.green, src:SOURCES.bsp,
                body:"Philippines imports ~30% of household consumption — all fuel, most wheat, many goods. When the peso weakens, import costs hit wholesale immediately. CPI captures it months later. Formula: peso depreciation % × 30% import share × 50% passthrough rate.",
